...
_Cukup chat santai di Telegram, seluruh transaksi otomatis diproses ke Web Dashboard analitik._
...
3. **Smart Categorization:** Mendeteksi otomatis 6 kategori default (_Makanan_, _Transportasi_, _Belanja_, _Tagihan_, _Hiburan_, _Kesehatan_) atau kategori kustom Anda.
...
- **🔗 Account Pairing Web ↔ Telegram:**
  ...
  - Otomatis memindahkan (_merge_) seluruh transaksi lama dari bot ke akun web Anda.
- **📱 Progressive Web App (PWA):**
  - Dilengkapi _Web App Manifest_, ikon vektor responsif, dan _custom install banner_.
...
| ------------ | ----------------------------- | --------------------------------------------------- |
| _Teks Bebas_ | `kopi susu 25k`               | Mencatat transaksi harian                           |
| _Perkalian_  | `2x 15k mie ayam`             | Mencatat dengan jumlah multiplier (Rp 30.000)       |
| _Mata Uang_  | `20 usd domain`               | Mencatat transaksi non-IDR                          |
...
Endpoint `/api/health` tersedia untuk memantau status sistem secara _real-time_:
